...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateCheckboxes(paramName) {
        const url = new URL(window.location.href);
        const ids = url.searchParams.getAll(paramName);

        document.querySelectorAll(`input[name="${paramName}"]`).forEach(function(checkbox) {
            checkbox.checked = ids.includes(checkbox.value);
        });
    }

    function updateAllCategoryCheckboxes() {
        updateCheckboxes('category_ids[]');
        updateCheckboxes('sub_category_ids[]');
        updateCheckboxes('sub_sub_category_ids[]');
        expandSelectedCategories();
    }

    function expandSelectedCategories() {
        document.querySelectorAll('input[name="category_ids[]"], input[name="sub_category_ids[]"], input[name="sub_sub_category_ids[]"]').forEach(function(checkbox) {
            if (checkbox.checked) {
                let parentLi = checkbox.closest('.categories-form-check');
                if (parentLi) {
                    let subMenu = parentLi.querySelector('.sub_menu');
                    if (subMenu) {
    ...
    function bindRealTimeActions() {
        document.querySelectorAll('.real-time-action-update').forEach(function(element) {
    ...
    document.addEventListener('ajaxProductsReloaded', function() {
        updateAllCategoryCheckboxes();
        bindRealTimeActions();
    });
});
