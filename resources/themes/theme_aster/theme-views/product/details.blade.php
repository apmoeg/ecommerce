    ...
    <div class="d-flex gap-4 flex-wrap align-items-center mb-3">
        ...
        <ul class="option-select-btn custom_01_option flex-wrap weight-style--two gap-2 pt-2">
            @foreach (json_decode($product->colors) as $key => $color)
                @php
                    $colorName = Color::where('code', $color)->value('name') ?? $color;
                    $colorNameFormatted = str_replace('-', ' ', $colorName);
                @endphp
                <li>
                    <label>
                        <input type="radio" hidden
                               ...
                               data-color-name="{{ $colorNameFormatted }}"
                               {{ $key == 0 ? 'checked' : '' }}>
                        <span
        ...
        <!-- Selected color name -->
        <div class="selected-color-name text-muted fw-semibold" id="selected-color-name">
            {{ translate('Color') }}: <span id="selectedColorName"></span>
        </div>
    </div>
@endif
...
@push('script')
    ...
    if (colorRadios.length && colorNameSpan) {
        function updateColorName() {
            const selected = document.querySelector('input[name="color"]:checked');
            if (selected && selected.dataset.colorName) {
                colorNameSpan.textContent = selected.dataset.colorName;
            }
        }
        ...
        colorRadios.forEach(radio => {
            radio.addEventListener('change', updateColorName);
        });
    }
});
</script>
