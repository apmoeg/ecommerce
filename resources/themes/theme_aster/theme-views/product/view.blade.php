...
@php
    ...
    $subSubCategoryId = request('sub_sub_category_id');

    if (request('data_from') === 'category') {
        if ($subSubCategoryId) {
            // If sub-sub-category exists, add sub-sub-category, sub-category, and category
            $subCategory = \App\Models\Category::where('id', $subSubCategoryId)->value('parent_id');
            $category = \App\Models\Category::where('id', $subCategory)->value('parent_id');

            if ($category) {
                echo '<input type="hidden" name="category_ids[]" value="'.$category.'">';
            }
            if ($subCategory) {
                echo '<input type="hidden" name="sub_category_ids[]" value="'.$subCategory.'">';
            }
            echo '<input type="hidden" name="sub_sub_category_ids[]" value="'.$subSubCategoryId.'">';
        } elseif ($subCategoryId) {
            // If only sub-category exists, add sub-category and category
            $category = \App\Models\Category::where('id', $subCategoryId)->value('parent_id');

            if ($category) {
                echo '<input type="hidden" name="category_ids[]" value="'.$category.'">';
            }
            echo '<input type="hidden" name="sub_category_ids[]" value="'.$subCategoryId.'">';
        } elseif ($categoryId) {
            // Only category
            echo '<input type="hidden" name="category_ids[]" value="'.$categoryId.'">';
        }
    }
@endphp
