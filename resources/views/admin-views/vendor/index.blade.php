...
<td>
    <div class="d-flex justify-content-center gap-2">
        ...
        {{-- زر تسجيل الدخول كتاجر --}}
        @if($seller->status === 'approved')
            <form method="GET" action="{{ route('vendor.auth.login') }}" target="_blank">
                @php \Illuminate\Support\Facades\Session::put('admin_authenticated', true); @endphp
                <input type="hidden" name="vendor_id" value="{{ $seller->id }}">
                <input type="hidden" name="admin_impersonate" value="1">
                <button type="submit"
                        class="btn btn-outline-primary btn-sm square-btn"
                        title="{{ translate('login_as_vendor') }}"
                        onclick="return confirm('هل تريد تسجيل الدخول كـ {{ $seller->f_name }}؟')">
                    <i class="tio-edit"></i>
                </button>
            </form>
        @endif
