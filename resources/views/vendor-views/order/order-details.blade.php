...
@section('content')
                        ...
                        <div class="d-flex flex-wrap gap-10 flex-md-nowrap justify-content-between mb-4">
                            ...
                            <div class="text-sm-right flex-grow-1">
                                <div class="d-flex flex-wrap gap-10 justify-content-end">
                                    ...
                                    @endif
                                    @if (getWebConfig('map_api_status') == 6 && isset($shippingAddress->latitude) && isset($shippingAddress->longitude))
                                        <div class="">
                                    ...
                                    <a class="btn btn--primary px-4 text-nowrap d-none" target="_blank"
    ...
    @if(isset($productDetails->code))
        <div>
            <strong>{{translate('MAIN_SKU')}} :</strong> {{$productDetails->code}}
        </div>
    @endif
    @if ($detail->variant)
        <div>
            <strong>{{translate('variation')}} :</strong> {{$detail['variant']}}
            @php($variantData = json_decode($productDetails->variation, true))
            @if($variantData)
                @foreach($variantData as $variant)
                    @if($variant['type'] == $detail->variant && isset($variant['sku']))
                        <br>
                        <strong>{{translate('SKU')}} :</strong> {{$variant['sku']}}
                    @endif
                @endforeach
            @endif
        </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="media">
                                ...
                                <div class="media-body d-flex flex-column gap-1">
                                    ...
                                    <span class="title-color break-all d-none"><strong>{{$order->customer['phone']}}</strong></span>
                                    <span class="title-color break-all d-none">{{$order->customer['email']}}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
@endsection
@push('script')
    @if(getWebConfig('map_api_status') ==6 )
        <script
