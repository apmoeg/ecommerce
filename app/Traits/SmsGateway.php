...
trait  SmsGateway
{
    public static function send($receiver, $otp): string
    {
        ...
        $config = self::get_settings('three_shm');
        if (isset($config) && $config['status'] == 1) {
            return self::three_shm($receiver, $otp);
        }
    ...
    public static function three_shm($receiver, $otp): string
    {
        $config = self::get_settings('three_shm');
        $response = 'error';
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://app.3shm.com/api/create-message',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
                ...
                CURLOPT_POSTFIELDS => array(
                    ...
                    'to' => $receiver,
                    'message' => str_replace("#OTP#", $otp, $config['otp_template']),
                    'sandbox' => 'false' // Optional parameter, depending on environment
                ),
            ));
