...
/*    public static function sales_commission($order)
    ...
    public static function sales_commission($order)
    {
        // استخدام العمولة المخزنة إن وجدت
        if (isset($order->admin_commission)) {
            return number_format($order->admin_commission, 2);
        }

        // حساب العمولة لو مفيش قيمة مخزنة
        $discount_amount = 0;
        if ($order->coupon_code) {
            $coupon = Coupon::where(['code' => $order->coupon_code])->first();
            if ($coupon) {
        ...
        $order_total = $order_summary['subtotal'] - $order_summary['total_discount_on_product'] - $discount_amount;
        $percentage_commission = self::seller_sales_commission($order['seller_is'], $order['seller_id'], $order_total);

        // حساب العمولة الثابتة
        $fixed_commission = getWebConfig(name: 'fixed_commission') ?? 0;
        $total_quantity = 0;
        foreach ($order->details as $detail) {
            $total_quantity += $detail->qty;
        }
        $fixed_commission_total = $fixed_commission * $total_quantity;

        // إجمالي العمولة
        $commission_amount = $percentage_commission + $fixed_commission_total;
    ...
    public static function sales_commission_before_order($cart_group_id, $coupon_discount)
    {
        ...
        $cart_summary = OrderManager::order_summary_before_place_order($carts, $coupon_discount);

        // Retrieve commission settings
        $fixed_commission = getWebConfig(name: 'fixed_commission') ?? 0; // Fixed amount per product unit
        ...
        $percentage_commission = self::seller_sales_commission($carts[0]['seller_is'], $carts[0]['seller_id'], $cart_summary['order_total']);

        // Calculate fixed commission
        $fixed_commission_total = $fixed_commission * $total_quantity;

        // Total commission
        $total_commission = $percentage_commission + $fixed_commission_total;
