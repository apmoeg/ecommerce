...
public function update(ProductUpdateRequest $request, ProductService $service, string|int $id): JsonResponse|RedirectResponse
{
    ...
    // Get initial dataArray
    $dataArray = $service->getUpdateProductData(request: $request, product: $product, updateBy: 'admin');

    // Update English fields directly into dataArray
    foreach (['name', 'description', 'title'] as $field) {
        if (!empty($request[$field])) {
            $dataArray[$field] = $request[$field][array_search('en', $request->lang)];
        }
    }

    // Remove old English translations manually
    $this->translationRepo->deleteEnglishTranslations(model: 'App\Models\Product', id: $id);
