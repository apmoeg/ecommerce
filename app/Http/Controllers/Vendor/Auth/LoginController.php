...
{
    if (request()->has('vendor_id') && request()->get('admin_impersonate') == 1) {
        if (!Session::has('admin_authenticated')) {
            abort(403, 'Unauthorized impersonation attempt.');
        }

        $vendor = $this->vendorRepo->getFirstWhere(['id' => request('vendor_id')]);
        if ($vendor && $vendor['status'] === 'approved') {
            \Illuminate\Support\Facades\Auth::guard('seller')->logout();
            \Illuminate\Support\Facades\Auth::guard('seller')->loginUsingId($vendor['id']);

            if ($this->vendorWalletRepo->getFirstWhere(params: ['seller_id' => $vendor['id']]) === false) {
                $this->vendorWalletRepo->add($this->vendorService->getInitialWalletData(vendorId: $vendor['id']));
            }

            Toastr::success(translate('Logged in as vendor'));
            return redirect()->route('vendor.dashboard.index');
        }

        Toastr::error(translate('Vendor not found or not approved'));
