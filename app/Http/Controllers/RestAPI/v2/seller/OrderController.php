...
public function details(Request $request, $id)
{
    ...
    if ($details->isEmpty()) {
        return response()->json([
            'errors' => [
                ['code' => 'order_details', 'message' => translate('No order details found!')]
            ]
        ], 404);
    }

    // Fetch the order and calculate admin commission
    $order = Order::find($id);
    if (!$order) {
        return response()->json([
            'errors' => [
    ...
    $adminCommission = Helpers::sales_commission($order);

    // Add admin_commission to each OrderDetail
    foreach ($details as $det) {
        $det['product_details'] = Helpers::product_data_formatting(json_decode($det['product_details'], true));
        $det['admin_commission'] = $adminCommission; // Append admin commission
    }
